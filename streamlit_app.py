...
_, view1, dwn1, view2, dwn2 = st.columns([0.20,0.30,0.20,0.20,0.20])
with view1:
...
with col7:
    st.markdown(
        "<h5 style='text-align: left; color: white; margin-bottom:-30px;'>Total Sales by Region and City in Treemap</h5>",
        unsafe_allow_html=True
    )
    st.plotly_chart(fig4, use_container_width=True)

    _, view4 , dwn4 = st.columns([0.5,0.5,0.5])
    with view4:
        result2 = df[["Region" , "City" , "TotalSales"]].groupby(by=["Region" , "City"])["TotalSales"].sum()
        expander = st.expander("Treemap Data")
...
_,view5, dwn5 = st.columns([0.65,0.5,0.5])
with view5:
